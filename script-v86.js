...
let lastPrivacyFocusedElement = null;
let activePrivacyCheckbox = null;
...
document.addEventListener('keydown', e => {
  ...
  if (privacyModal?.classList.contains('open')) closePrivacyModal();
  else if (modal?.classList.contains('open')) closeFormModal();
});
function openPrivacyModal(trigger=null) {
  lastPrivacyFocusedElement = document.activeElement;
  activePrivacyCheckbox = trigger?.closest('form')?.querySelector('input[name="privacyConsent"]') || null;
  privacyModal?.classList.add('open');
  privacyModal?.setAttribute('aria-hidden','false');
  document.body.classList.add('privacy-open');

  const content = privacyModal?.querySelector('.privacy-content');
  if (content) content.scrollTop = 0;
  window.requestAnimationFrame(() => {
    if (content) content.scrollTop = 0;
    privacyModal?.querySelector('#privacy-title')?.focus();
  });
}
function closePrivacyModal() {
  ...
  const returnFocus = lastPrivacyFocusedElement;
  activePrivacyCheckbox = null;
  returnFocus?.focus?.();
}
...
document.querySelectorAll('[data-close-privacy]').forEach(button => button.addEventListener('click', closePrivacyModal));
document.querySelector('[data-accept-privacy]')?.addEventListener('click', () => {
  if (activePrivacyCheckbox) {
    activePrivacyCheckbox.checked = true;
    activePrivacyCheckbox.dispatchEvent(new Event('change', {bubbles:true}));
  }
  closePrivacyModal();
});
...
async function buildPayload(form) {
  ...
  if (data.action === 'group') {
    ...
    if (!file.name.toLowerCase().endsWith('.xlsx')) throw new Error('團體名冊僅接受 .xlsx 格式。');
...
forms.forEach(form => form.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (payload.website) throw new Error('資料驗證失敗，請重新整理頁面後再試。');
    checkGroupCounts(payload);
...
if (['personal','group','payment'].includes(requestedForm)) {
  window.setTimeout(() => openFormModal(requestedForm), 0);
}
